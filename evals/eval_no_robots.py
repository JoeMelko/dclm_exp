...
HuggingFaceH4/no_robots dataset.

Each sample builds a prompt from the optional system `prompt` field and the
conversation history up to (but not including) the last assistant message. We
then score the model only on the last assistant response tokens (prompt
positions are masked).

Dataset is automatically retrieved from the HuggingFace dataset
"HuggingFaceH4/no_robots" if not already present locally.

Example
-------
python -m dclm_exp.clustering.eval_no_robots --uuid <RUN_UUID>
...
# --------------------------------------------------------------------------- #
# When executed within a package (e.g. `python -m dclm_exp.clustering.eval_no_robots`) the
...
def _extract_messages(conv_obj) -> List[Tuple[str, str]]:
    """Return list of (role, content) pairs from a messages field.

    Expected schema: list of dicts with keys {role, content}.
    """
...
def build_prompt_and_response(system_prompt: Optional[str], messages: List[Tuple[str, str]]) -> Optional[NoRobotsSample]:
    """Build prompt from `prompt` (system) + history up to last assistant; take that assistant as response.
    ...
    parts: List[str] = []
    # Prefer sample-level `prompt` as system instruction if provided
    if system_prompt and system_prompt.strip():
        parts.append("System:\n" + system_prompt.strip() + "\n")
    # Include any system/user/assistant messages from history
    ...
    parts.append("Assistant:\n")
    prompt_text = "\n".join(parts)
    return NoRobotsSample(prompt_text=prompt_text, response_text=response_text)
...
def collate(batch: List[NoRobotsSample], max_length: int) -> CollatedBatch:
...
#                           Dataset loader                                     #
# --------------------------------------------------------------------------- #
DATASET_REPO = "HuggingFaceH4/no_robots"
...
def _ensure_datasets_available():
    if load_dataset is None:
        raise RuntimeError(
            ...
            "to enable no_robots evaluation."
        )


def _load_split(split: str, total_samples: Optional[int]) -> List[NoRobotsSample]:
    _ensure_datasets_available()
    ds = load_dataset(DATASET_REPO, split=split)
    if total_samples is not None:
        ds = ds.select(range(min(total_samples, len(ds))))

    samples: List[NoRobotsSample] = []
    for ex in ds:
        system_prompt = ex.get("prompt", None)
        msgs = _extract_messages(ex.get("messages", None))
        built = build_prompt_and_response(system_prompt, msgs)
...
def main(args):
    ...
    def _collate_wrapper(samples: List[NoRobotsSample]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split", default="train",
                        help="Dataset split to evaluate (default: train; e.g., test)")
