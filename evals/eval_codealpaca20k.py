...
CodeAlpaca-20k instruction-following dataset.

Each sample forms a prompt from the instruction (and optional input), and we
score the model only on the output tokens (prompt positions are masked out).

Dataset is automatically retrieved from the HuggingFace dataset
"sahil2801/CodeAlpaca-20k" if not already present locally.

Example
-------
python eval_codealpaca20k.py --uuid <RUN_UUID>
...
# --------------------------------------------------------------------------- #
# When executed within a package (e.g. `python -m dclm_exp.clustering.eval_codealpaca20k`) the
...
#                         Tokenizer                                           #
# --------------------------------------------------------------------------- #
# Use the GPT-NeoX tokenizer to match Open-LM checkpoints as in eval_paloma
...
# --------------------------------------------------------------------------- #
@dataclass
class CodeAlpacaSample:
    instruction: str
    input_text: str
    output: str


def build_prompt(sample: CodeAlpacaSample) -> str:
    """Construct the text prompt given instruction and optional input."""
    parts: List[str] = []
    parts.append("Instruction:\n" + (sample.instruction or ""))
    if sample.input_text and sample.input_text.strip():
        parts.append("\nInput:\n" + sample.input_text)
    parts.append("\nResponse:\n")
    return "\n".join(parts)
...
def collate(batch: List[CodeAlpacaSample], max_length: int) -> CollatedBatch:
    """Tokenize and pad a batch of CodeAlpaca samples.

    - Input is prompt + output
    - Labels are -100 for prompt positions, and output token ids for response positions
    ...
    for s in batch:
        ...
        response_ids: List[int] = tokenizer.encode(s.output or "", add_special_tokens=False) + [tokenizer.eos_token_id]
...
#                           Dataset loader                                     #
# --------------------------------------------------------------------------- #
DATASET_REPO = "sahil2801/CodeAlpaca-20k"
...
def _ensure_datasets_available():
    if load_dataset is None:
        raise RuntimeError(
            ...
            "to enable CodeAlpaca-20k evaluation."
        )


def _load_split(split: str, total_samples: Optional[int]) -> List[CodeAlpacaSample]:
    _ensure_datasets_available()
    ds = load_dataset(DATASET_REPO, split=split)
    if total_samples is not None:
        ds = ds.select(range(min(total_samples, len(ds))))
    samples: List[CodeAlpacaSample] = []
    for ex in ds:
        samples.append(CodeAlpacaSample(
...
def main(args):
    ...
    def _collate_wrapper(samples: List[CodeAlpacaSample]):
