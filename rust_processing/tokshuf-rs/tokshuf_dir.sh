...
# Determine if we should pass the --use-tiktoken flag automatically. Currently
# the Rust binary has native support for a fast tiktoken implementation for the
# Neox-20B and Llama-3 tokenizers. Using it avoids a runtime panic that occurs
# when trying to fetch the tokenizer from the HuggingFace Hub (the model cards
# do not contain a tokenizer). We therefore enable the flag whenever one of
# these tokenizers is requested, unless the caller has already provided the flag
# explicitly via TOKSHUF_EXTRA.
NEED_TIKTOKEN="0"
case "$TOKENIZER" in
  "EleutherAI/gpt-neox-20b"|"meta-llama/Meta-Llama-3-8B")
    NEED_TIKTOKEN="1" ;;
esac
if [[ "$NEED_TIKTOKEN" == "1" && "$TOKSHUF_EXTRA" != *"--use-tiktoken"* ]]; then
  TOKSHUF_EXTRA="--use-tiktoken ${TOKSHUF_EXTRA}"
fi
