#!/usr/bin/env python
"""
prepare_openhermes_new.py
---------------------
Download *OpenHermes‑2.5*, format it so that loss / gradients apply **only to
assistant answers** using plain "User:"/"Assistant:" markers (no special chat
tokens), and emit tokenised WebDataset shards ready for DCLM
    ...
    for rec in ds:
        ...
        for turn in conv:
            ...
            if role == "gpt":
                answer = turn["value"]
                break
            # Treat any non-gpt role as a user turn; use plain markers with blank line separation
