...
* num_trainables    mem-map  (N_samples,)                 ~ int32
* index.tsv         row_id <TAB> sample_key             (deterministic order, but kept for debugging)

The order of samples is deterministic because the DataLoader uses a single
worker and no shuffling.  The index file is retained temporarily for
validation purposes.
"""
...
BATCH_COUNT_DTYPE = np.int32  # dtype for num_trainables mmap
# ------------------------------------------------------- #
...
def main(args):
    ...
    # Use a single worker to preserve exact sample order; this also eliminates the need for an index file.
    loader = wds.WebLoader(ds, batch_size=BATCH_SIZE, num_workers=0, collate_fn=collate)

    manifest_path = wds_dir_path / "manifest.jsonl"
    # No manifest – compute total rows from CLI args
    N_est = args.num_shards * args.shard_size

    # Accumulator for the FP32 sum of all per-example gradients
    sum_feats = np.zeros(
        (logger.grads.shape[0], logger.grads.shape[2]), dtype=np.float32
    )
    scaler = GradScaler()
    ...
    for step, (toks, labels, keys) in enumerate(tqdm.tqdm(loader, total = N_est // BATCH_SIZE)):
        ...
        batch_sum = features.sum(axis=0)  # sum over the batch dimension
        sum_feats += batch_sum
        logger.grads.zero_()
