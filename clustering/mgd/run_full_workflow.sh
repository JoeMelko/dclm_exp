...
#   5) run_collect_all.sh        – per-dataset cosine-similarity evaluation
#
...
DATA_PARENT_DIR="/home/jmelko/400m_tok"             # <- directory whose *sub-dirs* are processed by run_collect_all.sh
WDS_DIR="/home/jmelko/mgd/iter_1_dir/d_iter1_high_lr"                     # <- tokenised WebDataset root used by later stages
OH_DIR="/home/jmelko/openhermes_tok"

# -------- Model checkpoint ----- #
# Exactly *one* of UUID or CKPT must be defined.  Leave the unused one empty.
MODEL_UUID="iter1_high_lr-d=1024_l=24_h=8-warm=2000-lr=0p003-wd=0p033-cd=3e-05-bs=512-mult=1-seed=124-tokens=8232325120"         # Datacomp-LM / Open-LM run UUID (preferred)
...
# -------- Hessian computation ---#
GRAD_MEMMAP="/home/jmelko/mgd/iter_2_dir/grads.mmap"     # mem-mapped gradient features (input for hessian.py)
COND_TARGET=1e4                        # target condition number after ridge regularisation
WHITENERS_PATH="/home/jmelko/mgd/iter_2_dir/whiteners.npy"        # output file written by hessian.py
...
TARGET_DIR="targets"                  # directory that will contain sum_*.npy (created automatically)

# -------- Condition / whitening --#
WHITENED_TARGET="whitened_target.npy" # 1-D, unit-norm vector consumed by run_collect_all.sh

# --- Feature collection / mmap -- #
FEATURE_MEMMAP="/home/jmelko/mgd/iter_2_dir/grads.mmap"         # destination memmap initialised by create_mmap_features.py
...
# -----------------------------------------------------------------------------
# 5) Cosine-similarity collection (run_collect_all.sh)
# -----------------------------------------------------------------------------

echo "[workflow] Stage 5/5 – run_collect_all.sh"

"${PROJECT_DIR}/run_collect_all.sh" "${DATA_PARENT_DIR}" \
