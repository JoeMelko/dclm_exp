...
Each sample forms a prompt from the problem/instruction text (and optional
context), and we score the model only on the generated solution tokens (prompt
positions are masked out). We intentionally target the model's step-by-step
"generated_solution" field rather than any gold/expected final answer fields.

Dataset is automatically retrieved from the HuggingFace dataset
"nvidia/OpenMathInstruct-2" if not already present locally.

Example
-------
python eval_openmathinstruct2.py --uuid <RUN_UUID>

Optional arguments
------------------
--batch-size      Evaluation batch size (default: 8)
--max-length      Maximum sequence length after tokenization (default: 2048)
--split           Dataset split to evaluate (default: train_1M)
...
# --------------------------------------------------------------------------- #
# When executed within a package (e.g. `python -m dclm_exp.clustering.eval_openmathinstruct2`) the
...
# Prefer the canonical OMI-2 fields shown on the dataset card/viewer
# https://huggingface.co/datasets/nvidia/OpenMathInstruct-2/viewer/default/train_1M
_PROMPT_KEYS = [
    "problem", "instruction", "question", "prompt", "input", "task", "query", "text"
]
# Only use the generated solution for supervision
_RESPONSE_KEYS = [
    "generated_solution"
]
_CONTEXT_KEYS = [
...
def _extract_fields(record: dict) -> Optional[OMISample]:
    prompt_text: Optional[str] = None
    for k in _PROMPT_KEYS:
        if k in record and isinstance(record[k], str) and record[k].strip():
            prompt_text = record[k].strip()
            break

    # Optional: include context if present
    context_text: Optional[str] = None
    for k in _CONTEXT_KEYS:
        if k in record and isinstance(record[k], str) and record[k].strip():
            context_text = record[k].strip()
            break

    response_text: Optional[str] = None
    for k in _RESPONSE_KEYS:
        if k in record and isinstance(record[k], str) and record[k].strip():
            response_text = record[k].strip()
            break

    if not prompt_text or response_text is None:
        return None

    # Build a structured prompt header to match other evals
    prompt_parts: List[str] = ["Instruction:\n" + prompt_text]
    if context_text:
        prompt_parts.append("\nContext:\n" + context_text)
    prompt_parts.append("\nResponse:\n")
    prompt_full = "\n".join(prompt_parts)
    return OMISample(prompt_text=prompt_full, response_text=response_text)
...
def collate(batch: List[OMISample], max_length: int) -> CollatedBatch:
    prompt_id_seqs: List[List[int]] = []
    response_id_seqs: List[List[int]] = []

    for s in batch:
        p_ids = tokenizer.encode(s.prompt_text, add_special_tokens=False)
        r_ids = tokenizer.encode(s.response_text or "", add_special_tokens=False) + [tokenizer.eos_token_id]
        p_ids, r_ids = _truncate_for_max_length(p_ids, r_ids, max_length)
        if not r_ids:
...
def _ensure_datasets_available():
    if load_dataset is None:
        raise RuntimeError(
            ...
            "to enable OpenMathInstruct-2 evaluation."
        )


def _load_split(split: str, total_samples: Optional[int]) -> List[OMISample]:
    _ensure_datasets_available()
    # Splits include: 'train_1M', 'train', 'train_2M', 'train_5M'
    ds = load_dataset(DATASET_REPO, split=split)
    if total_samples is not None:
        ds = ds.select(range(min(total_samples, len(ds))))

    samples: List[OMISample] = []
    for ex in ds:
        built = _extract_fields(ex)
...
def main(args):
    ...
    def _collate_wrapper(samples: List[OMISample]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split", default="train_1M",
                        help="Dataset split to evaluate (default: train_1M; e.g., train, train_2M, train_5M)")
