...
Each sample forms a prompt from the instruction (and optional context), and we
score the model only on the response tokens (prompt positions are masked out).

Dataset is automatically retrieved from the HuggingFace dataset
"databricks/databricks-dolly-15k" if not already present locally.

Example
-------
python eval_dolly15k.py --uuid <RUN_UUID>
...
# --------------------------------------------------------------------------- #
# When executed within a package (e.g. `python -m dclm_exp.clustering.eval_dolly15k`) the
...
def build_prompt(sample: DollySample) -> str:
    """Construct the text prompt given instruction and optional context.

    The format encourages consistent conditioning across tasks.
    """
    parts: List[str] = []
    parts.append("Instruction:\n" + (sample.instruction or ""))
    if sample.context and sample.context.strip():
        parts.append("\nContext:\n" + sample.context)
    parts.append("\nResponse:\n")
    return "\n".join(parts)
...
def collate(batch: List[DollySample], max_length: int) -> CollatedBatch:
    """Tokenize and pad a batch of Dolly samples.
    ...
    for s in batch:
        ...
        response_ids: List[int] = tokenizer.encode(s.response or "", add_special_tokens=False) + [tokenizer.eos_token_id]
...
def _ensure_datasets_available():
    if load_dataset is None:
        raise RuntimeError(
            ...
            "to enable Dolly 15k evaluation."
        )


def _load_split(split: str, total_samples: Optional[int]) -> List[DollySample]:
    _ensure_datasets_available()
    ds = load_dataset(DATASET_REPO, split=split)
    if total_samples is not None:
        ds = ds.select(range(min(total_samples, len(ds))))
    samples: List[DollySample] = []
    for ex in ds:
        samples.append(DollySample(
...
def main(args):
    ...
    def _collate_wrapper(samples: List[DollySample]):
