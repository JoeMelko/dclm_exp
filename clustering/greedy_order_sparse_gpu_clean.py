...
def greedy_gpu_sparse(counts_all_gpu: torch.Tensor, counts_sparse_gpu: torch.Tensor,
    ...
    pbar = tqdm(total=n_total, desc="Ordering sequences (GPU-sparse)")
    ...
    while len(order) < n_total:
        ...
        dots = torch.sparse.mm(counts_sparse_gpu, residual_for_choice.unsqueeze(1)).squeeze(1)
...
def main():
    ...
    ap.add_argument('--discard-ratio', type=float, default=0.0,
                    help='Ratio of sequences to discard from the end after ordering (0..1]. Writes the first 100*(1-ratio)% to the dataset')
    args = ap.parse_args()
    ...
    if discard_ratio > 0.0:
        keep_ratio = max(0.0, min(1.0, 1.0 - discard_ratio))
        keep_n = int(len(order_indices) * keep_ratio)
        if keep_n < len(order_indices):
            orig_len = len(order_indices)
